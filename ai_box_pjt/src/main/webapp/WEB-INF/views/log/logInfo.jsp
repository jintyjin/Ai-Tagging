...
url += 'https://211.117.45.82:8000/cgi-bin/login_chk.cgi';
var form = document.createElement('form');
form.setAttribute('method', 'post');
form.setAttribute('action', url);
form.setAttribute('target', '_blank');
...
viewer_url = 'RMSviewer64://sigma?policy=' + 'a' + '&ip=' + ip + '&port=' + data.dev_port + '&id=' + data.dev_id + '&hash=' + SHA256(data.dev_pwd) + '&newver=' + '1.0.0.475' + '&newurl=' + encodeURIComponent(mainUrl + RMS_INSTALLER_PATH);
...
/* function connectViewer(row, caller) {
var data = $('#products').jqxGrid('getrowdata', row);

var type = $('#ipType' + row + ' option:selected').val();
var ip = '';
...
if(isInternetExplorer()) {
	setViewer(ip, data.client_port, data.device_hash, policy);

	switch(caller) {
	case CONNECT_SELECT_VIDEO:
		openViewer('viewer_web.htm', 'webViewer');
		break;
	case CONNECT_SELECT_MONITOR_MIRRORING:
		openViewer('viewer_remote.htm', 'remoteViewer');
		break;
	default:
		break;
	}
	...
	if(getOS() === 'Windows') {
		...
		case CONNECT_SELECT_VIDEO:
			if(is64bitOS())
				viewer_url = 'RMSviewer64://sigma?policy=' + policy + '&ip=' + ip + '&port=' + data.client_port + '&id=admin&hash=' + data.device_hash + '&newver=' + RMS_INSTALLER_VERSION + '&newurl=' + encodeURIComponent(mainUrl + RMS_INSTALLER_PATH);
			else
				viewer_url = 'RMSviewer86://sigma?policy=' + policy + '&ip=' + ip + '&port=' + data.client_port + '&id=admin&hash=' + data.device_hash + '&newver=' + RMS_INSTALLER_VERSION + '&newurl=' + encodeURIComponent(mainUrl + RMS_INSTALLER_PATH);

			viewer_path = RMS_INSTALLER_PATH;
			break;
		case CONNECT_SELECT_MONITOR_MIRRORING:
			if(is64bitOS())
				viewer_url = 'RemoteViewer64://sigma?policy=' + policy + '&ip=' + ip + '&port=' + data.client_port + '&id=admin&hash=' + data.device_hash;
			else
				viewer_url = 'RemoteViewer86://sigma?policy=' + policy + '&ip=' + ip + '&port=' + data.client_port + '&id=admin&hash=' + data.device_hash;

			viewer_path = '/RemoteViewer_installer.exe';
			break;
		default:
			break;
		}
...
$(document).ready(function () {
	...
	/* obj.id = "user";
	obj.pw = "1234"; */
    obj.search_date = get('searchdate');
					...
					buttonclick: function (row) {
						...
						if (rowData.event_source == 'SCADA') {
							...
							if (get('userdata') != null) {
								...
								popup_window[POPUP_SEARCH] = openWindow('/logPopup?dev_ch=' + rowData.dev_channel + '&event_time=' + rowData.event_time, 'logPopup', 1280, 720);	// 1280 + 16, 720 + 68
							}
						}
...
function change_text() {
	$("#searchDate").text(get('searchdate'));
}
</script>
</head>
<body>
<div class="container" style="overflow:auto;">
	<div id = "Progress_Loading" align="center" ><!-- 로딩이미지 -->
		...
		<div id="searchDate" style="font-size:14px; margin-top:20px;"></div>
